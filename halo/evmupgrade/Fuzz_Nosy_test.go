...
func Fuzz_Nosy_EventProcessor_deliverCancelUpgrade__(f *testing.F) {
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		var _x3 *bindings.UpgradeCancelUpgrade
		fill_err = tp.Fill(&_x3)
		if fill_err != nil {
			return
		}
		...
		p.deliverCancelUpgrade(ctx, _x3)
	})
}

func Fuzz_Nosy_EventProcessor_deliverPlanUpgrade__(f *testing.F) {
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		var plan *bindings.UpgradePlanUpgrade
